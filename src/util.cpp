...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LMToken
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LMToken
    // Mac: ~/Library/Application Support/LMToken
    // Unix: ~/.LMToken
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LMToken";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "LMToken.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty LMToken.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "LMTokend.pid"));
